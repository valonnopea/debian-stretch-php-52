...
RUN \
	curl --silent \
		--output "/tmp/php-${PHP_VERSION}-fpm-${FPM_VERSION}.diff.gz" \
		"https://php-fpm.org/downloads/php-${PHP_VERSION}-fpm-${FPM_VERSION}.diff.gz" && \
	cd "/tmp" && \
	gunzip "/tmp/php-${PHP_VERSION}-fpm-${FPM_VERSION}.diff.gz" && \
	cd "/usr/src/php-${PHP_VERSION}" && \
	patch --strip="1" \
		< "/tmp/php-${PHP_VERSION}-fpm-${FPM_VERSION}.diff"

# From http://storage.googleapis.com/google-code-attachments/php52-backports/issue-16/comment-2/libxml29_compat.patch
COPY \
	"patches/libxml29_compat.patch" \
	"/tmp/libxml29_compat.patch"
...
# Self-made patch
COPY \
	"patches/gmp-__GMB_BITS_PER_MP_LIMB.patch" \
	"/tmp/gmp-__GMB_BITS_PER_MP_LIMB.patch"
...
RUN \
	bash "/tmp/fix-makefile.sh" "/usr/src/php-${PHP_VERSION}" && \
	rm "/tmp/fix-makefile.sh"
...
ENTRYPOINT [ "/opt/php/5.2.17/bin/php-cgi" ]
CMD [ "--fpm", "--fpm-config", "/opt/php/5.2.17/etc/php-fpm.conf" ]
